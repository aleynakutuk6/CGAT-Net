...
def display_segmented_sketch(pixel_similarity_array,binary_sketc,classes_colors,class_ids,img_id,save_path=None):
    ...
    colormap = np.zeros((class_indices.shape[0], class_indices.shape[0], 1), dtype=np.uint8)
    ...
    for i, color in enumerate(classes_colors):
        ...
        colormap[mask] = class_ids[i]
        
    mask_tensor_org = binary_sketch[:,:,0]/255
    hsv_image[mask_tensor_org==1] = [0,0,1]
    
    colormap[mask_tensor_org==1] = 0
    scipy.io.savemat(os.path.join(save_path, "CLASS_PRED", "sample_" + img_id + "_class.mat"), {'CLASS_PRED': colormap[:,:,0]})
    ...
    for i, class_name in enumerate(classes):
        ...
        if np.any(mask):
            ...
            txt = plt.text(
                centroid_x, centroid_y, class_name, color=classes_colors[i], ha='center', va='center',fontsize=14,   # color=classes_colors[i]
    ...
    if save_path:
        ...
        plt.savefig(os.path.join(save_path, "DRAWING_PRED", "L0_sample" + img_id + ".png"), bbox_inches='tight', pad_inches=0)
...
def get_segmentation_map(pixel_similarity_array, binary_sketch, class_ids):
    # Find the class index with the highest similarity for each pixel
    class_indices = np.argmax(pixel_similarity_array, axis=0)
    # Create an HSV image placeholder
    colormap = np.zeros((class_indices.shape[0], class_indices.shape[0], 1), dtype=np.uint8)
