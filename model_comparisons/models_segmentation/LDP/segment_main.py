...
def segment_main(**kwargs):
    ...
    if not ckpt and not kwargs['ckpt_file']:
        ...
        if kwargs['ckpt_file']:
            ...
            start_iter = int(ckpt_file.split("_")[-1].replace(".tfmodel", ""))
            print("Starting from {}".format(start_iter))
        else:
    ...
    if mode == 'train':
        ...
        # datasets/SketchyScene-7k/colorMap.mat
        colorMap = scipy.io.loadmat('../datasets/SketchyScene-7k/colorMap.mat')['colorMap']
        print("colorMap shape", colorMap.shape)
        outstr = mode + ' mode\n'
        cat_max_len = 16

        hist = np.zeros((FLAGS.nSketchClasses, FLAGS.nSketchClasses))
        data_name = FLAGS.data_base_dir.split("/")[2]
        ...
        for imgIndex in range(1, nImgs + 1):
            ...
            label_path = os.path.join(FLAGS.data_base_dir, mode, 'CLASS_GT_changed', label_name)
            ...
            classes_dir = os.path.join(eval_base_dir, data_name, 'CLASS_PRED')            
            ...
            if imgIndex == nImgs:
                ...
                log_str = '\nRound {}, CRF({}), edgelist({}) dataset {}'.format(imgIndex, use_dcrf, use_edgelist, data_name)
                print(log_str)
                outstr += log_str + '\n'
                ...
                outstr += '\n>>> IoU of each class' + '\n'
                for classIdx in range(colorMap.shape[0]):
                    if FLAGS.ignore_class_bg:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--mode', '-md', type=str, choices=['train', 'val', 'test', 'inference'],
                        default='test', help="choose a running mode")
    parser.add_argument('--infer_dataset', '-infd', type=str, choices=['val', 'test'],
                        default='test', help="choose a dataset for inference")
    ...
    parser.add_argument('-rn', '--run_name', type=str, default="LDP_SS")
    parser.add_argument('--ckpt_file', '-cf', type=str, 
                        default='outputs/snapshot/LDP_SS/iter_150000.tfmodel', help="checkpoint file to restore (without suffix)")
    # 'outputs/snapshot/LDP_SS/iter_150000.tfmodel'
