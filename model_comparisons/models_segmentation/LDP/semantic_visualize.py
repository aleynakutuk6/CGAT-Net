...
def visualize_semantic_segmentation(label_array, color_map, black_bg=False, save_path=None):
    ...
    # visual_image_sel = np.zeros((label_array.shape[0], label_array.shape[1], 3), dtype=np.uint8)
    # visual_image_sel[label_array == 11] = [(255, 255, 255)]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset', '-ds', type=str, choices=['train', 'val', 'test'],
                        default='test', help="choose a dataset")
    parser.add_argument('--image_id', '-id', type=int, default=29, help="choose an image")
    parser.add_argument('--black_bg', '-bl', type=int, choices=[0, 1],
                        default=1, help="use black or white background for visualization")
    parser.add_argument('--data_basedir', '-db', type=str, default='../datasets/FreeHand', help="set the data base dir")
    ...
    label_path = os.path.join(args.data_basedir, args.dataset, 'CLASS_GT_changed', label_name)
    label = scipy.io.loadmat(label_path)['CLASS_GT']
    label = np.array(label, dtype=np.int32)  # shape = [H, W]
    # print("label", label)
    # print("label shape", label.shape)
    label_set = set(label.flatten().tolist())
    
    """
    labels_dict = {}
    for el in label.flatten():
        if el in labels_dict:
            labels_dict[el] += 1
        else:
            labels_dict[el] = 1
    """
